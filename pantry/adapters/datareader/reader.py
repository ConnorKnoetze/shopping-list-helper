    ...
    def read_data(self):
        ...
        with open(RECIPE_DATA_FILE, mode="r", newline="", encoding="utf-8") as csvfile:
            ...
            for row in recipes_reader:
                ...
                recipe_ingredients = [
                    tuple(ing.split(",")) for ing in row["ingredients"].split(";")
                ]
                recipe_methods = [
                    method.strip() for method in row["method"].strip().split("||")
                ]
                recipe_prep_time_mins = (
                    int(row["prep_time_mins"].strip())
                    if row["prep_time_mins"].strip()
                    else None
                )
                recipe_cook_time_mins = (
                    int(row["cook_time_mins"].strip())
                    if row["cook_time_mins"].strip()
                    else None
                )
                recipe_total_time_mins = (
                    int(row["total_time_mins"].strip())
                    if row["total_time_mins"].strip()
                    else None
                )
