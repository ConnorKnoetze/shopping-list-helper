...
def remove_from_shopping_api(name: str):
    ...
    if ing in user.grocery_list:
        ...
        return jsonify({"success": True, "message": f"{name} removed from grocery list.", "name": name}), 200
    else:
        return jsonify({"success": False, "message": f"{name} not found in grocery list.", "name": name}), 404
